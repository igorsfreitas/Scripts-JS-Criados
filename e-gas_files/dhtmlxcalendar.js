function dhtmlXCalendarObject(inps, skin, grid, coluna) {
	
	this.meugrid = grid;
	this.minhacoluna = coluna;
	...
	this.contDates.onclick = function(e){
		...
		if (t._date != null && !t._css_dis) {
			...
			if (that._activeInp && that.i[that._activeInp]) {
				...
				// o codigo de setar vem aqui
			
				
				if(that.meugrid != null){ 	
					that.meugrid.cells(that.meugrid.getSelectedRowId(),that.minhacoluna).setValue(that._dateToStr(new Date(that._activeDate.getTime())));
					//meugrid.setCellTextStyle(meugrid.getSelectedRowId(),minhacoluna,"backgroundcolor:"+b[0]+";");
				}
...
dhtmlXCalendarObject.prototype.langData = {
	"en": {
		dateformat: "%Y-%m-%d",
		monthesFNames: ["Janeiro","Fevereiro","Mar&ccedil;o","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"],
		monthesSNames: ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"],
		daysFNames: ["Domingo","Segunda-Feira","Ter&ccedil;a-Feira","Quarta-Feira","Quinta-Feira","Sexta-Feira","Sabado"],
